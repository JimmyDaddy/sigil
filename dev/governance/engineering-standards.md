...
覆盖率门禁默认要求 workspace 单测行覆盖率 `>= 96%`，由 `cargo-llvm-cov` 执行；CI 和本地必须使用同一个脚本入口，避免阈值或统计范围漂移。默认统计范围排除少量 orchestration loop / adapter 文件，具体列表以 `scripts/coverage.sh` 为准；这些文件只能承载入口调度、provider I/O 桥接和启动失败出口，不应成为新增业务逻辑的覆盖率逃逸区。
...
- 真实本地配置使用仓库根目录或用户配置目录下的 `sigil.toml`，该文件可能包含密钥，默认不提交；配置示例以 `README.md` 的“配置要点”为准
