...
- managed writer 在 admission 后必须写入 authority-owned owner-only admission marker；finalize 前必须把真实 `records.jsonl` byte length、record count 与 content hash 提交给 Resource Authority，由 authority journal 记录 physical frontier 后才能 settlement。current boot 遇到无 settlement 的 admission 时，必须通过 authority-owned physical verifier 按 `Observed -> StartedShadow -> RecoveryOperationPrepared -> ResolutionPrepared -> RecoveryOperationSettled -> ResolutionSettled -> DomainBlockerProjected` 顺序恢复；marker 缺失、frontier 不完整或 prefix 不一致必须 fail closed，不得从 surface error 或目录扫描猜造 no-effect
