...
| `windows-sys` | `0.61.2`；仅 Windows target；`Win32_Storage_FileSystem,Win32_Foundation,Win32_Security,Win32_System_JobObjects,Win32_System_Threading` | `sigil-http/durable_io`、`sigil-process` | HTTP durable journal 使用 `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)`；`sigil-process` 为 tools 与 stdio MCP 的每个本地 child 建立 kill-on-close Job Object，assignment 失败即回收 direct child 并 fail closed，取消/timeout/output failure 通过 `TerminateJobObject` 回收已登记进程树；`Win32_Security` 只为 `CreateJobObjectW` 的 nullable `SECURITY_ATTRIBUTES` 签名提供类型 | `MIT OR Apache-2.0`；Microsoft/windows-rs | 仅在 `cfg(windows)` 编译；内部 `sigil-process` crate 统一 ownership 实现而未引入新第三方版本；Job Object 只证明 lifecycle ownership，不声明 filesystem、network 或 restricted-token sandbox；Unix 保持既有 rename/fsync 与 process-group 实现 |
