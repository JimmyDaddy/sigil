...
| `security-framework` | `3.7.0`；仅 macOS target | `sigil-runtime/provider_connections/keyring_store` | `auto` 在 file miss 时用 per-query `kSecUseAuthenticationUISkip` 查询或清理旧 generic-password record；不修改 process-global Keychain UI 状态，不写 native record，也不复制 secret | `MIT OR Apache-2.0`；kornelski/rust-security-framework | 该版本已由 `keyring` 的 macOS backend 间接锁定；现作为 target-specific direct dependency 复用。认证必需、Keychain 不可用或记录缺失均不得弹 UI；非认证类错误仍 fail closed |
