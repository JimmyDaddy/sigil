...
- Skill index snapshots and loaded-skill summaries now have `SkillIndexCaptured` / `SkillLoaded` control entries and are projected through `Session::skill_state_projection`; runtime discovery now covers `.sigil/skills`, `.sigil/commands/*.md`, `.sigil/agents`, explicitly enabled compatibility sources under `.claude/skills`, `.claude/agents`, and `.reasonix/agents`, and optional user skills, including frontmatter parsing, shadowing warnings, hashing, and invalid path/name rejection; `.sigil/commands/*.md` entries are discovered by default as user-invocable inline commands and run through the same trusted inline skill invocation path behind `/command-id`; internal read-only `load_skill` validates enabled/trusted/model-invocable state and permission policy, reads only the skill entrypoint, injects the loaded skill body as current-run transient context, and appends a `SkillLoaded` control entry; the TUI `/config` `Agents` section now uses the workspace-aware `AgentProfileRegistry` to display built-in, native, compatibility, and plugin-contributed profiles, including source/kind/trust/effective enabled/user/model, provider/model, tool scope, and nickname candidates, while the primary footer only exposes trust/disable; lower-level enabled/user/model policy decisions remain append-only control entries but are not part of the ordinary user flow; the `Skills` section displays only inline/reusable skills and commands, including enabled/trust/source/hash/run mode/tool scope/path patterns, and exposes only one primary footer action, use; use opens an optional instructions input and submits a request still governed by the runtime `load_skill` policy; TUI slash fallback also lists only trusted inline skills/commands, so `run_as=child_session` compatibility resources are no longer displayed as ordinary skill slash rows or resolved through `/skill-id`; composer-leading `@` opens an agent mention selector that lists only enabled, trusted, user-invocable agent profiles; submitting `@profile <prompt>` goes through the TUI worker `InvokeAgentProfile` command and runtime `AgentToolRuntime::invoke_agent_profile`, starts a foreground child thread with `AgentInvocationSource::Mention`, validates enabled/trusted/user-invocable policy, and does not rely on the ordinary chat delegation hard gate; native and plugin agent profiles support alias/slash metadata with deterministic conflict warnings; plugin manifest discovery covers `.sigil/plugins/<id>/plugin.toml`, manifests can contribute agents, skills, hooks, and MCP servers, and the TUI `/config` Plugins section displays manifest path, id/name/version, skills/hooks/MCP commands, hash, and execution implications, with footer approve/deny actions that append `PluginManifestCaptured` and `PluginTrustDecision` control entries. Only a plugin whose session trust decision matches the current manifest hash can feed agent registration into `AgentProfileRegistry`, using namespaced profile ids.
- Native workspace agent profiles support an OpenCode-style `permission` field: `.sigil/agents/<id>/agent.toml` accepts `[permission]` shorthand / per-key tables, and `.sigil/agents/<id>/AGENT.md` frontmatter accepts a nested `permission:` map. These rules append after root permission and use last-match-wins semantics, so agent rules can override global allow/ask/deny. `tool_scope` / `allowed_tools` only narrow tool visibility and do not grant permission; root `read-only` mode, protected paths, destructive operations, external-directory gates, and changeset-only foreground write isolation remain fail-closed hard boundaries. TUI `/config` agent detail shows permission mode, tool override count, rule count, external-directory state, and write policy summaries without exposing a complex rule editor.
