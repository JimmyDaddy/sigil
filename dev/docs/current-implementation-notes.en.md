...
TUI `/config` exposes only high-frequency provider fields, permissions, memory, compaction, code intelligence controls, terminal mouse/OSC52/scroll sensitivity compatibility settings, the Skills browser, and common MCP server fields. It can switch between `deepseek` and `openai_compat`; DeepSeek FIM is shown as a provider-specific advanced field, while OpenAI-compatible marks it unsupported. Lower-frequency provider-specific fields remain available through config files and environment variables. The Skills browser runs discovery against the current workspace/config, supports PgUp/PgDn skill switching, shows trust/source/hash/run mode/invocable/tool scope/path patterns, and uses footer load/invoke actions to submit requests still governed by the runtime `load_skill` policy.
