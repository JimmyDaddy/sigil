...
- Skill index snapshots and loaded-skill summaries now have `SkillIndexCaptured` / `SkillLoaded` control entries and are projected through `Session::skill_state_projection`; runtime discovery, `load_skill`, and the TUI skill browser remain later P1 phases.
