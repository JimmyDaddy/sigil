...
- `sigil-http` currently owns the HTTP/SSE adapter server config DTO and boundary tests; later work will add HTTP routing, auth, SSE serialization, and the runtime session/run registry without depending on `sigil-tui` or duplicating the agent loop.
