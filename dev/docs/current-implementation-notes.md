...
runtime delegate 在 final answer 前会检查 agent thread projection：非 terminal 的 `join_before_final` child thread 会阻止最终回答并要求 `wait_agent` 或 `read_agent_result`；background child thread 未完成时，只有最终回答明确说明后台仍在运行才允许返回。delegate 还会从 durable session entries 汇总 approvals、tool commands/gates、subagents 和 changed files 作为 `session_facts`，用于阻止过于泛泛的最终回答；模型需要基于记录的 facts 汇报验证和变更。`spawn_agent` 会对父级最近 user request 与 child objective/prompt 中的路径/模块 token 做保守重叠检测，发现明显同 scope 时发出 warning，提醒 parent 只继续非重叠工作或在 final 前读取 child 结果。
