...
- skill index snapshot 和 skill loaded 摘要已有 `SkillIndexCaptured` / `SkillLoaded` control entry，并通过 `Session::skill_state_projection` 投影；runtime discovery、`load_skill` 和 TUI skill browser 仍是后续 P1 阶段。
