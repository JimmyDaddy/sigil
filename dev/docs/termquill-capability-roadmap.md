...
当前 `termquill` 已经具备一个可用的 TUI-first coding agent 闭环：

- `termquill-kernel`：provider、tool、session、approval、permission、event、memory 和 compaction 的通用契约。
...
| P0 | MCP 完整闭环 | tools 可用；trust enforcement 与 TUI 手动 lazy activation 已落地；elicitation 未完成 | 支持安全 elicitation、MCP lifecycle status 和按需 server 激活 |
...
```bash
cargo test -p termquill-kernel config permission
cargo test -p termquill-tui config modal setup
cargo test -p termquill-mcp
...
- 新 crate：`crates/termquill-code-intel`
- 初始职责：workspace 扫描、语言检测、LSP client 生命周期、符号/诊断缓存、model-visible 摘要。
- 暂不放进 `termquill-kernel` 公共契约；等工具接口稳定后再决定是否上移。
...
```bash
cargo test -p termquill-code-intel
cargo test -p termquill-tui provider_status view_model
cargo check
```
...
```bash
cargo test -p termquill-tools-builtin
cargo test -p termquill-tui ui::timeline
cargo test -p termquill-kernel permission agent
```
...
```bash
cargo test -p termquill-mcp
cargo test -p termquill-kernel permission
cargo test -p termquill-tui timeline_flow_tests
```
...
- `elicitation/create` 当前明确返回 unsupported，避免挂死。

交付物：

1. kernel 增加 provider-neutral 的 user elicitation event 或 tool-side interaction request。
2. TUI 增加 modal，展示 MCP server、请求字段、默认值、风险提示。
3. Headless CLI 默认返回明确错误；可选支持 `--elicitation-policy deny|json-file`。
4. 所有 elicitation 决策写入 append-only control state。

验收标准：

- MCP server 请求用户输入时，TUI 不挂起、不伪造输入。
- 用户拒绝时 server 收到明确 error。
...
- TUI `/config` 的 MCP section 已在 footer actions 中提供 `activate`；worker 空闲时可把已保存的 lazy server 热接入当前 agent registry，运行中会拒绝并给出可诊断错误。
- 模型不会在 activation 前看到不可调用伪工具。

交付物：

1. 启动时记录 lazy server 配置状态，但不暴露伪工具给模型。（crate activation API 已落地；TUI lifecycle metadata 可继续细化）
2. 用户或模型需要某 MCP capability 时触发 activation。（TUI 手动触发已落地；模型触发待设计）
3. activation 成功后工具进入 registry；失败按 required / optional 策略处理。（已落地）
4. TUI 显示 MCP server lifecycle。（配置页 summary 已落地；运行态 status 可继续细化）
...
1. 新 crate：`crates/termquill-provider-openai-compat`。
...
```bash
cargo test -p termquill-provider-openai-compat
cargo test -p termquill-runtime
cargo test -p termquill-kernel agent session
cargo test -p termquill-tui config slash session
```
...
- [`termquill-tui-mouse-interaction-design.md`](termquill-tui-mouse-interaction-design.md)
...
cargo test -p termquill-tui approval_flow_tests
cargo test -p termquill-tui shell
```

## 9. Phase 5：Planner / Executor / Subagent

目标：让 Termquill 支持更长、更复杂的任务，同时保持 prefix cache 和 session audit 清晰。
...
```bash
cargo test -p termquill-tools-builtin edit write
cargo test -p termquill-kernel approval session
cargo test -p termquill-tui approval timeline
```
...
1. `termquill run --json` 输出 newline-delimited events。
...
2. `cargo install` 文档。
3. Homebrew formula。
4. `termquill doctor` 或 TUI diagnostics panel：
   - config path
   - provider auth
   - workspace root
   - MCP server status
...
2. **Code intelligence foundation**：建立 `termquill-code-intel`，先让 Rust 项目诊断和符号可用。
