...
- 不承诺尚未实现的 `sigil serve`、checkpoint/rewind、session lifecycle 或稳定外部 API。
...
如果公开文档必须表达限制，应写可验证的产品效果，例如“当前 `sigil serve` 只检查配置，尚不启动本地服务”，而不是解释 adapter 的内部阶段。实现与协议细节应迁移到对应 RFC、技术方案或 `dev/docs` 专题文档，并从公开页链接到用户可执行的替代路径，而非链接到源码。
