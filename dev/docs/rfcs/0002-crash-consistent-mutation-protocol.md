...
- 已将 persistent terminal 自然退出路径接入 owner-pushed lifecycle 与 mutation reconciliation：terminal owner 在状态变化时主动追加 `TerminalTask` 终态，并基于 `terminal_start` 的 `ExecutionMutationProfile` 追加 precise `WorkspaceMutationDetected` 或 unknown-dirty evidence；TUI worker steady state 不再周期读取 active terminal 状态。
