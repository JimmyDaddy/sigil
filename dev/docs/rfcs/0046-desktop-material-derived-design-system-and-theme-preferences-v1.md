...
状态：complete / R46.0-R46.8 complete
...
3. 增加 application-scope 的 `system` 与冻结命名 palette 主题偏好、native 持久化、首帧防闪烁和系统主题运行时跟随。
...
- **Native durable owner**：保存 application-scope `ThemePreference`，在窗口 visible 前把具体 palette 派生为 Tauri native
  light/dark theme；旧 `light | dark` 文件值只在读取时迁移为 Sigil palette。
- **Renderer presentation owner**：根据偏好与当前 OS theme 计算 `ResolvedTheme`，设置 `html[data-theme]`、
  `html[data-color-scheme]`、CSS `color-scheme` 和 theme-color；所有 React component 只消费 semantic role/context。
...
R46.3 的实现已完成：native owner 在 application config directory 使用 4 KiB 上限、versioned、deny-unknown、atomic-replace
的 `appearance-v1.json`；窗口创建前应用 `system | light | dark`，并只向 pre-paint initializer 注入冻结枚举。Renderer 通过
external self-hosted bootstrap 在 React/CSS 之前设置 `data-theme-preference`、resolved `data-theme` 与 `color-scheme`，再由
root-level `ThemeProvider` 原位同步 native snapshot。唯一新增 capability 是收窄的 appearance command；没有增加 generic window、
filesystem、path、process、network、bearer 或 OpenAPI/runtime capability。

前端验证已通过：22 个 Vitest tests（包含 active conversation draft identity、保存失败保持上一 proven state、scoped retry、
system event 与 `Cmd/Ctrl+,`）、UI system gate、TypeScript typecheck 和 Vite production build。Theme bootstrap 另有静态 gate，阻止
inline script、storage/network/bridge/token 能力和 production catalog 泄漏。

早期的 macOS `dyld::_dyld_start` host failure 已恢复；R46.7 重新执行后，`cargo test -p sigil-desktop-app`
的 23 个 native tests（包含 3 个 appearance tests）全部通过，`cargo clippy -p sigil-desktop-app --all-targets -- -D warnings`
与 `cargo fmt --all --check` 也通过。`pnpm --dir apps/desktop package` 完成 release sidecar、Tauri app 和 DMG 打包，
因此 native store、resolver、Tauri command 与真实 package 的证据已齐全，R46.3 记为 complete。
...
R46.6 已完成 conversation 与 contextual review 迁移。App 级 selected-session banner 已删除，conversation title、recorded-run count
与 stream state 只由 `ConversationPanel` header 呈现。Verification 不存在时不渲染 Review action、空 inspector 或第二 grid track；证据存在时，
expanded（≥1280px）使用 inline complementary pane，medium/compact（≤1279px）使用 repo-owned Drawer。Drawer 的 Esc、关闭按钮、焦点恢复
均由同一 modal owner 管理，测试同时证明 timeline node/scrollTop 和 composer node/draft 不会因 review 开关 remount。

ApprovalDock 仍固定在主 conversation flow，blocking run/transcript error 不依赖 inspector。Composer、approval decision、transcript paging、
verification rerun/evidence copy、diff copy、message copy 与 shared error action 已全部迁入 internal primitive；raw-interactive migration ledger
现在为空，legacy `useFocusBoundary` 已删除。ToolCard 不再为缺失 duration/risk 伪造“not recorded/not classified”文案，只展示实际存在的
optional metadata。

R46.2 的 `--color-*`、`--space-*`、`--radius-*`、legacy z/motion/shadow alias 已全部删除，真实 surface 与 primitive 直接消费
`--sg-sys-*` / `--sg-domain-*` role；UI gate 会拒绝 alias 回流。Conversation domain CSS 已从 monolithic shell 拆到
`features/conversation/conversation.css`。验证通过：29 个 frontend tests（含 single-final、reattach、approval、cancel、verification、
IME/draft、review continuity）、UI system/raw-control/legacy-token/focus-debt gate、TypeScript typecheck、Vite production build 与
`git diff --check`。本片未改变 renderer DTO、capability、OpenAPI 或 runtime contract。

## 24. R46.7 result

R46.7 已建立可机械复核的 cross-theme/adaptive/accessibility evidence matrix。Dev-only catalog 现在覆盖
no-workspace、empty catalog、30/100 sessions、degraded sources、running tool + high-risk approval、reconnect/gap、failed verification +
unified diff、long EN/ZH copy 和 missing optional metadata；每个 fixture 都声明 system/light/dark、normal/forced-colors、
full/reduced motion、1280/900/840/839/760/320 viewport 和 100%/200% zoom 矩阵。UI gate 额外冻结 320px 无 document
horizontal scroll、1279/839 breakpoint、forced-colors role/shadow、reduced-motion、远程 CSS asset 禁止与 production catalog
unreachability。

自动验证通过：31 个 frontend tests、UI system gate、TypeScript/Vite production build、high-severity dependency audit、
sidecar package tests、23 个 native desktop tests、strict Clippy 和 Rustfmt。真实 `Sigil.app` 与 DMG 打包成功，
`codesign --verify --deep --strict` 通过；通过 LaunchServices 后台冷启动证明生产 app 进程与 1120×760
WebView window 成功建立。冷启动没有抢占前台，但 macOS 将隐藏启动的 window 放在非当前 Space，该状态只对
AX client 暴露 application/menu bar，不暴露 WebView content。因此本次只记录已执行的 AX probe，不将它写成
VoiceOver 内容导航 pass；当前 Space 的 VoiceOver/manual focus traversal、notarization、Gatekeeper distribution assessment
以及 Windows/Linux 真实 WebView 继续作为公开 installer 发布闸门，不阻塞已自动证明的 R46.7 实现关闭。

## 25. R46.8 completion audit

R46.8 以 findings-first 方式审查 RFC-0046 全部变更，共发现并修复 2 个 P1 与 2 个 P2：

- **P1 / platform floor**：生产 bundle 保留 `Array.prototype.at` 与 `String.prototype.replaceAll`，Vite 不会为冻结的
  Safari 13 floor 注入 runtime polyfill。现已改为 index/split-join 实现，UI gate 会拒绝重新引入，生产 bundle
  也执行了实际字符串检查。
- **P1 / 320px usability**：紧凑顶栏的 Browse、workspace、new conversation 和 Appearance 最小宽度之和会裁切操作。
  320px 下现在只收起次要文字，保留四个可访问操作与 workspace label，并增加静态回归 gate。
- **P2 / overlay accessibility**：popover dialog 缺少稳定 accessible name，menu/workspace 选择后可能因 panel 卸载丢失焦点，
  嵌套 modal 的下层 dialog 仍留在 AX tree。现已补齐 panel name、选择后 trigger focus restore，并以 stack-aware
  body isolation 确保只有 topmost modal 对辅助技术可见。
- **P2 / catalog completeness**：typed fixture 只能被测试 import，没有供人工 WebView smoke 使用的可运行页面。现已增加
  `pnpm --dir apps/desktop dev:catalog` 和独立 `catalog.html`，可切换 fixture、theme、viewport、contrast、motion 和
  zoom；它不 import desktop bridge，production graph/bundle marker gate 会阻止泄漏。

修复后未发现剩余 P1/P2。验证通过：32 个 desktop frontend tests、UI system/Safari-floor/catalog-exclusion gate、
TypeScript 与 Vite production build、OpenAPI/generated TypeScript drift、high-severity pnpm audit、sidecar preparation，以及
`cargo fmt --all --check`、`cargo check`、full workspace tests、strict all-target Clippy 和 workspace Rustdoc。EN/ZH 用户状态文档、
Pages 首页、developer ownership 规则与 desktop dogfood guide 已同步，docs/Pages gates 通过；Pages 的可选 macOS
dark-media Chrome emulation 因未生成完整 DOM 按现有非强制策略跳过，未写成 pass。

真实 macOS `Sigil.app` 与 DMG 在最终代码上重建，sidecar `--version`、strict deep codesign 与 packaged catalog
exclusion 通过。Apple notarization、public Gatekeeper distribution assessment、当前 Space 的 VoiceOver 内容导航、Windows/Linux
真实 system-WebView 与公开 installer 签名仍保持独立 release gate；RFC-0046 不把这些未执行项写成已通过。
