...
- 不采集 prompt、assistant reply、thought、tool input/output、文件内容、diff、环境变量值、API key、配置文件正文或 session JSONL。doctor 已产生的非秘密 provider/model、MCP alias、环境变量名称和 capability 摘要会在预览中明确列为 included diagnostic metadata。
...
- `checks[]`: status、stable name、message、可选 remediation
...
`privacy` 必须诚实列出 doctor checks 可能保留的非秘密配置摘要：provider/model label、MCP alias、credential environment variable name、capability/sandbox status。它只能声称排除配置文件正文、credential/environment value、conversation/tool/file content，不能笼统声称“无配置数据”。
...
- 投影按 stable check category 使用字段级 allowlist，不允许把现有 doctor 的任意 message/remediation 原样透传。每个允许字段再经过调用方提供的 credential redactor 和现有 safe-persistence 文本投影，最后替换已知 config/workspace/cache/state/home 绝对路径为稳定占位符。
...
- 明确排除：conversation、tool input/output、file content/diff、config file/credential/environment values；同时明确 doctor metadata 可能包含 provider/model、MCP alias 与环境变量名称；
- 当前报告大小和 check 数；
- `Enter` 导出、`Esc` 取消。

第一次 Enter 才写盘到 Sigil cache 下的 `support-bundles/`。文件名由 runtime 生成，不接受用户 path；目录权限在 Unix 上为 `0700`、文件为 `0600`，destination 使用 create-new/atomic publish，拒绝 symlink parent 和覆盖现有文件。成功后 modal 显示准确路径、GitHub issue URL、`C` 复制 URL 与“附加前复核”的提醒。
...
- `/feedback` modal 独占输入；Enter、C、Esc 不进入 composer。
