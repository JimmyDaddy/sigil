...
状态：accepted / R42.0 implemented；R42.1-R42.5 pending
...
- [`rusqlite`](https://github.com/rusqlite/rusqlite) 提供同步、参数化的 SQLite binding；V1 使用
  `bundled` feature，使 macOS/Linux/Windows desktop 分发不依赖目标机预装的 SQLite ABI。所有调用都在
  runtime/HTTP 的 blocking boundary 内执行。
...
event/checksum，显式 rebuild 无条件重读并重新计算所有 bounded source。V1 不把 metadata fast path 宣称为
同用户恶意篡改检测器：能够同时改写 source bytes、mtime 和 SQLite 的本地同用户攻击者不在 catalog threat
model 内。若 scan 不完整、目录不可验证或 transaction 失败，不执行 stale-row deletion。
...
quiescent；其他 Sigil process 持有 lease 或活动 connection 时 fail closed。随后才把数据库及 `-wal`/`-shm`
sidecar 移到同目录的 timestamped quarantine，再创建空 V1 并从 JSONL rebuild。自动 HTTP query 不执行
quarantine/delete；恢复必须是显式 owner action或 startup policy，不能形成 old/new database split-brain。
