...
Session-scoped runtime configuration, including TUI `/model`, now uses `boot_current_schema_with_config`: the runtime receives the surface-owned validated `RootConfig` and observes the persisted file only for its private source identity. It no longer reloads the persisted model and silently discards the session override; the persisted default remains unchanged. Production cutover construction is current-schema-only through `evaluate_current_schema`; epoch-selecting evaluation and Legacy rehydration are test-only historical fixtures, while Legacy wire data remains inspectable as unavailable diagnostic data.
