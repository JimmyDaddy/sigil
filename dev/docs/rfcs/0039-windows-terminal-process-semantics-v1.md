...
回收 direct child，且本次工具调用失败，不允许静默退回仅 direct-child kill。
...
PTY 由 `portable-pty` 创建进程，V1 在 spawn 返回后立即 assignment，并在 receipt 中保持这一
边界；不把它扩写成 suspended-spawn race-free sandbox。若 assignment 失败，PTY start 失败并调用
direct killer 收尾。
