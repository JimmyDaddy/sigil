...
```bash
git tag -a v0.0.1-alpha.5 -m "Sigil 0.0.1-alpha.5"
...
9. Publish a GitHub release with archives, checksum files, `checksums.txt`,
   ...
   prerelease suffix, such as `v0.0.1-alpha.5`, are published as GitHub
   prereleases.
...
gh release download v0.0.1-alpha.5 \
...
git commit -m "chore: update sigil-ai to 0.0.1-alpha.5"
...
```bash
scripts/generate-release-notes.sh v0.0.1-alpha.5
```
