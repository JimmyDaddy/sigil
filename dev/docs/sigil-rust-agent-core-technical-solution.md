...
- `/config` agent detail 会展示 permission 摘要和 write policy 摘要，包括 mode、tool override count、rule count、external-directory 状态以及 write-capable profile 是否只能通过 changeset-only foreground merge 写入；复杂 rule 编辑仍留给配置文件，不进入默认 footer action。
