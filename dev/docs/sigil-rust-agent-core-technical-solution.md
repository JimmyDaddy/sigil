...
- 主会话 running-input queue 使用 `ConversationInputQueued`、`ConversationInputEdited`、`ConversationInputReordered`、`ConversationInputStatusChanged` 和 `ConversationInputQueueControl` append-only control entry 持久化。普通 chat 和 `/plan <prompt>` 在 active run busy 时进入 queue，不提前写入 provider-visible user history；worker 在当前 turn 结束后 FIFO dispatch，成功写 `Delivered`，失败写 `Rejected` 并 pause queue。`/queue next` 只调整顺序等待下一 turn；`/queue now` 先走 cancel/interrupted audit，再 dispatch 选中 queued item。
- Background child result completion 与 queue 有明确优先级：`join_before_final` / blocking child 完成后优先触发 parent continuation；普通 non-blocking background child 完成只写 `AgentResultContinuation(Pending)` ready 状态。当主会话已经有 queued input 时，non-blocking result 不抢占 queued input，只以 bounded transient system notice 提醒模型可按需 `wait_agent` / `read_agent_result`。
...
- MCP tool/resource/prompt 输出必须先本地脱敏，再按默认 byte/line 限额截断，并在 `ToolResultMeta` 中保留 truncation 与 MCP server/tool/trust/operation metadata。
