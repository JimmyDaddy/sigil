...
- Task DAG 当前只完成 read-only ready batch selection；`SequentialTaskOrchestrator` 仍按 step 顺序执行。真实并发需要先把 child execution 从 parent `Session` 可变借用中拆出，并由 completion-driven coordinator 串行提交 parent control entries，属于 RFC-0053 O4-O5。
