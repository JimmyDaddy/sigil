...
进度已完成。到达顺序只保存在 `AgentSupervisor` 生命周期内的 live snapshot；parent 继续在全部
terminal envelope 收口后按 request sequence 单写 durable commit，TUI task strip/info rail
显示 `arrival #N → commit #M`，不会把运行态 arrival 顺序写入 session 或恢复授权。
