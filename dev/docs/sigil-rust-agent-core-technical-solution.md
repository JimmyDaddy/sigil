...
- `sigil-http`：HTTP/SSE adapter crate。当前承载 server config DTO、in-memory session/run registry、run start/cancel 与 approval decision routing；后续只做 HTTP routing、auth 与 SSE serialization，不依赖 `sigil-tui`，不复制 agent loop。
