...
- `message_agent` 目前不注册为 model-visible tool；runtime delegate 只对有效目标记录 requested -> rejected 的 `AgentThreadMessageRouted` 审计并返回 unsupported，直到 active child-agent mailbox 支持落地前不承诺 live steering 已送达。
