...
Planner / executor / subagent 协作已经作为 TUI-first task flow 落地。Durable task 的显式入口是 TUI `/task <任务>`；`/plan` 只表示一次性 Plan mode / read-only planning prompt，不创建 durable task state。TUI 中 `task.routing_policy = "auto"` 时，普通 chat 可以通过 host-bound `request_task_planning` handoff 进入同一 durable task flow；默认 `manual` 仍保持 chat-first。Production HTTP driver 与 Desktop-owned `sigil serve` child 已附加共享 foreground task executor，显式 `auto` 可以完成不需要额外 Task 控制动作的 planner/executor/synthesis 流程。Shared application control 已复用 TUI 的 exact `TaskPauseRequest` 校验和 Task stop transition：请求前绑定 task/plan/scope，只有 root execution、child/effect permit 全部 quiescent 后才通过单一有序 writer batch 追加 active step/child terminal，并最后追加 Task `Paused` / `Cancelled`；cleanup、join 或最终 binding 不确定时只能追加 `Interrupted`。普通 run cancel 只有在 durable cancellation scope 真实绑定 Task 时才修改该 Task，不能误伤旧任务。HTTP schema v9 已通过 authenticated typed route、幂等 command receipt 和 production supervisor 接入同一 Task pause authority；只有 shared application control 完成 durable finalization 后才确认 `Paused`，不确定时只能记录 `Interrupted`。Desktop schema v9 handshake、native typed client、Tauri allowlist command 与 Task card 已消费同一 capability 和 exact pause route，Pause 不复用普通 Stop/Cancel。HTTP/Desktop 的 guidance、integration review/accept 与 restart control 仍需接线，因此还不能宣称完整 application parity，兼容默认值也不切换。恢复只补本地 handoff/TaskRun admission crash gap，不重放原 conversation provider request；只有能证明尚未发生 planner/participant dispatch 的 task 才自动接管，stale Running step/lease 会先记为 Interrupted/Paused，再由 `/task continue` 显式继续。`/plan continue` 不再作为 alias。普通 chat 明确要求 subagent / 子 agent delegation 时，可通过 agent-thread tools 直接创建 child agent，不需要进入 durable task。
