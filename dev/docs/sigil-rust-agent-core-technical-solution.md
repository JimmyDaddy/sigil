...
- TUI `/config` 的 MCP section 提供 `activate` action；worker 空闲时可对已保存的 lazy server 执行 activation，并把真实工具加入当前 agent registry，运行中 activation 会被拒绝；模型也可通过 `mcp_activate_server` 工具按需启动指定 lazy server，成功后下一轮 request 会看到真实 MCP tools；lifecycle summary 会展示 `deferred`、`activating`、`ready` 或 `failed` 运行态
