...
- `apps/desktop`：RFC-0044 的 Tauri 2 + React/TypeScript/Vite companion。Tauri backend通过`sigil-desktop`维持one process per workspace，renderer只接收workspace id/display/server state等收窄DTO；capability仅允许冻结的desktop command，不开放generic shell/process/filesystem/HTTP。wire schema由`sigil-http` OpenAPI导出并在CI检查snapshot和generated TypeScript drift，renderer不直接持有loopback client或bearer。
