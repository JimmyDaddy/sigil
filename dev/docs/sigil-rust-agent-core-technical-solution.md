...
- O6g promotion review 的 artifact 前置已完成：changeset-only 与 worktree child 的 exact diff 在离开 child runner 前进入 RFC-0002 内容寻址 artifact lifecycle；durable proposal facts 不再只引用进程内 `inline:sha256` 内容，后续 review/promotion 可在 session ownership 迁移后重新读取并校验原字节。
