...
写入 exact Plan approval、稳定 Task identity和 first-class `TaskDirectExecutionAdmittedV1`，runner立即使用完整
approved Plan作为 objective；不创建隐藏的单步 TaskPlan，也不再调用 model/compiler生成第二份 Task DAG。
两个以上真实 Plan label可以形成 display-only checklist，但它不授予调度或完成权威。workspace snapshot 是 audit observation，
不是 invocation authority；无关 drift 会 rebase，只有 effect-local read/write set或 path CAS冲突才阻塞该 effect。
...
R71.9n closes the authority-input and TUI snapshot split found in the follow-up audit. Production boot accepts only the persisted `config_path` plus captured `launch_cwd`; no public seam accepts a caller-supplied complete `RootConfig` for authority composition. The current pointer uses a stable config-path application identity, while a durable, locked generation record in the config parent carries persisted configuration changes, including storage-root changes. TUI `/model` emits only a `ResolvedModelRoute` session overlay. Persisted configuration remains the sole input for authority roots and CAS operations, while the worker receives a separate effective session configuration. The normal-dependency shipping test covers persisted roots, unchanged cutover publication, and the subsequent permission-toggle CAS sequence. Implementation gates pass, but a new exact-SHA local release wrapper and hosted five-platform qualification are still required.
