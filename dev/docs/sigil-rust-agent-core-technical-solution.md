...
- O6f promotion protocol 已建立独立安全检查点，但物理 promotion 尚未完成：`TaskPromotionPreview` 只从全部 terminal-ready、已有 scoped receipt 且 cleanup disposition 明确的 lanes 生成，并绑定 ordered candidates、aggregate diff、单一 target、verification invalidation、intent/policy 与 digest；host-owned `TaskPromotionAuthorityConsumed` 再绑定 expiry 和 single-use nonce，普通 TaskPlan、planner 文本或 tool approval 不能替代。当前唯一启用来源是 exact user integration review；RFC-0005 E05.17 deferred 时 controlled-auto source fail closed。promotion attempt 与 parent verification 分别 append-only，只有 authoritative promoted snapshot 上的 RFC-0003 `Passed` receipt 才能满足 `synthesis_ready_attempt`；aggregate workspace mutation、Git ref CAS、parent check runner 和 task synthesis 接线继续属于 O6f 后续切片。
