...
`sigil` 的主入口是 TUI，而不是命令集合。当前 `sigil-tui` 已使用 `ratatui` + `crossterm`，并且 TUI 入口已经启用 mouse capture；事件循环也已经能收到 `CrosstermEvent::Mouse`。当前已产品化的鼠标行为主要是滚轮滚动：
...
`crates/sigil-tui/src/main.rs` 的主循环应从直接匹配 scroll 改为：
