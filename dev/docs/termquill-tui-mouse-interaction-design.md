# Termquill TUI 鼠标交互功能设计

状态：设计草案

本文定义 `termquill-tui` 支持鼠标交互时的产品边界、功能范围、事件模型、状态模型、实现分层、测试策略和分阶段交付计划。
...
- [`termquill-rust-agent-core-technical-solution.md`](termquill-rust-agent-core-technical-solution.md)

## 1. 背景

`termquill` 的主入口是 TUI，而不是命令集合。当前 `termquill-tui` 已使用 `ratatui` + `crossterm`，并且 TUI 入口已经启用 mouse capture；事件循环也已经能收到 `CrosstermEvent::Mouse`。当前已产品化的鼠标行为主要是滚轮滚动：

- 有待审批内容时，滚轮滚动 approval preview / diff
- 没有待审批内容时，滚轮滚动 timeline

这说明底层能力不是问题。真正缺少的是一套可维护的 TUI 鼠标交互语义：坐标命中、区域优先级、点击动作、状态转换、安全约束、提示同步和测试覆盖。
...
1. 鼠标事件处理只属于 `crates/termquill-tui`。
2. 不向 `termquill-kernel` 暴露 UI 坐标、鼠标事件或 provider 私有语义。
...
1. 文本选择、复制、拖拽选择。
2. 拖动改变面板大小。
3. hover tooltip 或 hover preview。
4. 双击、三击、多指手势。
5. 依赖 GUI 桌面事件的能力。
6. 鼠标直接操作 terminal 原生 scrollback 中的历史消息。

这些能力可以作为后续扩展，但不进入第一轮设计闭环。
...
5. 点击 allow / deny action：
   ...
   - 只接受左键 release 或左键 down 中的一种，必须全局统一
...
1. 点击 composer 进行光标定位。
2. 点击 tool card header 直接展开或折叠。
3. 支持 alternate scroll sensitivity 配置。
4. 支持 terminal capability 检测与禁用 mouse capture。
5. 支持 hover highlight，但只作为视觉反馈，不触发业务动作。

## 6. 信息架构与区域模型

鼠标支持需要把当前 renderer 内的临时布局计算提取为可复用模型。建议新增：

```text
crates/termquill-tui/src/mouse.rs
crates/termquill-tui/src/ui/layout_snapshot.rs
```
...
第一版不建议新增，避免焦点模型过早变复杂。

### 8.2 鼠标选择状态

第一版不需要单独持久化鼠标状态。所有可见选择应复用已有字段：
...
| ApprovalFileList | previous file row | next file row |
...
2. 清除 activity-only transient notice 不强制要求
3. 不提交、不清空、不移动光标

Phase 4 才考虑按点击坐标定位 input cursor。
...
- 如果用户第二次点击同一个候选行，可以执行；第一版不启用，避免误触。

### 9.4 点击 tool card

行为：

1. 命中 tool card header/body 时，设置 `selected_tool_timeline_entry`
2. 不自动展开
3. `Ctrl-O` 仍负责展开/折叠
...
- 点击 header：选中
- 点击 header 中的 expand affordance：展开/折叠
...
不建议第一版支持点击 permission card 直接切换写权限。写权限切换属于风险动作，保留 `BackTab` 或明确按钮后再做。
...
1. 只响应左键单击。
2. 推荐统一使用 `LeftUp` 触发，减少按下后拖出区域的误触。
3. 如果实现无法可靠处理 `LeftUp`，则使用 `LeftDown`，但必须写入测试并保持所有按钮一致。
4. action 区域必须是 footer 中明确 badge 或按钮所在 rect。
...
3. 点击 footer action：选择 footer action，不直接保存；再次 Enter 保存。
4. 如果要支持直接点击保存，必须把 footer action 做成明确按钮并测试 dirty guard。
...
`crates/termquill-tui/src/main.rs` 的主循环应从直接匹配 scroll 改为：
...
- `termquill-tui` 状态模型
- `termquill-tui` renderer layout helper
- `termquill-tui` tests
...
3. `mouse_click_composer_focuses_composer`
4. `mouse_click_slash_candidate_selects_entry`
5. `mouse_click_tool_card_selects_card`
6. `mouse_click_info_rail_agent_row_shows_detail`
7. `mouse_click_approval_file_selects_file`
8. `mouse_click_approval_allow_returns_approval_action`
9. `mouse_click_approval_deny_returns_approval_action`
10. `mouse_click_behind_approval_modal_is_noop`
...
1. `README.md`
   - TUI 交互说明
   - approval 操作说明
2. `dev/docs/termquill-rust-agent-core-technical-solution.md`
...
```bash
cargo fmt --all --check
cargo test -p termquill-tui mouse_scroll
cargo check -p termquill-tui
```
...
```bash
cargo fmt --all --check
cargo test -p termquill-tui mouse_
cargo check -p termquill-tui
```
...
```bash
cargo fmt --all --check
cargo test -p termquill-tui approval
cargo test -p termquill-tui mouse_
cargo check -p termquill-tui
```
...
```bash
cargo fmt --all --check
cargo test -p termquill-tui setup
cargo test -p termquill-tui config
cargo test -p termquill-tui session
cargo test -p termquill-tui mouse_
cargo check -p termquill-tui
```
...
1. 鼠标按钮统一用 `LeftDown` 还是 `LeftUp` 触发？
   - 建议 approval action 用 `LeftUp`
   - 普通 selection 可用 `LeftDown`
2. slash candidate 是否支持第二次点击执行？
   - 建议第一版不支持
3. 点击 tool card 是否直接展开？
   - 建议第一版只选中，展开继续走 `Ctrl-O`
...
`termquill-tui` 可以支持类似现代 TUI coding agent 的鼠标辅助体验，但应该按“区域命中 + 状态转换 + 安全审批”的方式做，而不是在主循环里直接按坐标写业务分支。
...
4. 最后扩展 setup/config/session 和可选增强
