...
use super::{
    ...
    HTTP_TOOL_ARTIFACT_PAGE_SCHEMA_VERSION, HttpApplicationCacheUsage,
    ...
    HttpPlanAction, HttpPlanDecisionAction, HttpPlanDecisionCommandReceipt,
    HttpPlanDecisionRequest, HttpPlanReview, HttpPlanReviewDetail, HttpPlanReviewSource,
    ...
    HttpSessionSnapshot, HttpSessionTranscriptMessage, HttpSessionTranscriptPage, HttpSseError,
    ...
    HttpTerminalLifecycleView, HttpTerminalTaskCancelRequest, HttpToolArtifactPage,
    ...
    HttpUserInputDecisionCommandReceipt, HttpUserInputDecisionDriverCommand,
...
async fn provider_setup_starts_without_config_saves_exact_route_and_reuses_catalog() {
    ...
    assert_eq!(inventory["connections"], json!([]));
    ...
    .await;
    assert_eq!(status, 200);
    assert_eq!(catalog["connection_id"], "openai-compatible-1");
    assert_eq!(catalog["models"][0]["model_id"], "local-coder");
    ...
    assert_eq!(
        saved["default_model"],
        json!({
            "connection_id": "openai-compatible-1",
            "model_id": "local-coder"
        })
    );
...
async fn provider_setup_saves_an_explicit_model_when_catalog_auth_is_rejected() {
    ...
    let (provider_base_url, _, provider_server) = spawn_provider_catalog_server(
        401,
        r#"{"error":{"message":"provider-upstream-secret-canary"}}"#,
    )
    .await;
    let temp = tempfile::tempdir().expect("temporary directory should open");
    let config_path = temp.path().join("sigil.toml");
    ...
    .await;
    assert_eq!(status, 200);
    assert_eq!(catalog["state"], "auth_rejected");
    assert!(!catalog.to_string().contains(secret_canary));
...
async fn local_server_projects_typed_run_context() {
    ...
    driver.set_run_context_view(HttpRunContextView {
        ...
        cache_usage: Some(HttpApplicationCacheUsage {
            cache_read_tokens: 3_000,
            cache_miss_tokens: 1_000,
            cache_write_tokens: Some(250),
            last_layout_mutation: Some("conversation_tail_appended".to_owned()),
    ...
    assert_eq!(body["cache_usage"]["cache_read_tokens"], 3_000);
    assert_eq!(
        body["cache_usage"]["last_layout_mutation"],
...
async fn spawn_provider_catalog_server(
    status: u16,
    body: &'static str,
) -> (String, Arc<AtomicUsize>, tokio::task::JoinHandle<()>) {
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
        .await
        .expect("provider catalog listener should bind");
    let address = listener.local_addr().expect("provider catalog address");
