...
pub(crate) fn render_inspected_group_lines(
    entries: &[(usize, &TimelineEntry)],
    options: &TimelineRenderOptions,
) -> Vec<Line<'static>> {
    let accent = accent_rose();
    let mut lines = vec![inspected_group_header_line(entries.len())];
    for (_, entry) in entries {
        ...
        let expanded = options.expand_tool_previews
            || options.expanded_tool_activity_keys.contains(&activity.key)
            || (default_expanded && !options.collapsed_tool_activity_keys.contains(&activity.key));
        lines.push(inspected_group_member_line(
            &display,
            selected,
            expanded,
            options.max_content_width,
...
fn inspected_group_header_line(count: usize) -> Line<'static> {
    let accent = accent_rose();
    Line::from(vec![
        Span::styled(
            "▎",
        ...
        Span::raw("  "),
        Span::styled(format!("{count} items"), Style::default().fg(dim())),
    ])
}

fn inspected_group_member_line(
    ...
    let mut spans = vec![
        Span::styled("▎", Style::default().fg(accent_rose())),
        Span::raw("   "),
    ];
    spans.extend(tool_title_spans(
        &display.title,
        tool_title_width(display, max_content_width.saturating_sub(2)),
    ));
    spans.push(Span::raw("  "));
