...
async fn run_main(cli: Cli) -> Result<u8> {
    ...
    let Some(command) = cli.command else {
        sigil_tui_host::launcher::run_tui_with_build_context(
            cli.config,
            build.into(),
    ...
    match command {
        ...
        Commands::Resume { session } => {
            sigil_tui_host::launcher::run_tui_resume_with_build_context(
                cli.config,
                session,
                build.into(),
                build.update_metadata(),
    ...
    build_doctor_report_with_options(
        ...
        DoctorReportOptions {
            appearance_checks: Some(
                &sigil_tui_host::appearance_diagnostics::appearance_doctor_checks,
            ),
