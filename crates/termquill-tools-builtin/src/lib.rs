...
fn limit_text_head_tail(input: &str, max_bytes: usize) -> TextLimitResult {
    ...
    content.push_str(&format!(
        "[termquill: output truncated, omitted {omitted_bytes} bytes]\n"
    ));
...
fn truncate_line_for_model(line: &str) -> String {
    if line.chars().count() <= MAX_MODEL_LINE_CHARS {
        ...
        truncated.push_str("[termquill: line truncated]");
        truncated
    }
}
...
fn append_truncation_notice(output: &mut String) {
    ...
    output.push_str(
        "[termquill: output truncated; use offset/limit or a narrower path/pattern to continue]",
    );
}
