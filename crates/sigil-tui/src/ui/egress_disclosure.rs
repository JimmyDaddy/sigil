...
use crate::app::EGRESS_DISCLOSURE_HEIGHT;
use crate::surface::EgressDisclosureCard;
...
/// Render a disclosure card from the immutable surface projection.  A successful draw is
/// acknowledged by the launcher after `Terminal::draw`, never by this renderer.
pub(crate) fn render_active_egress_disclosure_card_surface(
    frame: &mut Frame,
    area: Rect,
    card: Option<&EgressDisclosureCard>,
    theme: &Theme,
) -> bool {
    ...
    let lines = vec![
        ...
        Line::from(card.destination.clone()),
        Line::from(format!(
            "{} · {} · active",
            card.route, card.data_categories
        )),
