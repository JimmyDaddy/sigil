...
fn focused_terminal_task_cancel_requires_confirmation() -> Result<()> {
    ...
    assert!(matches!(
        second,
        Some(AppAction::CancelTerminalTask { identity })
            if identity.task_id == "terminal-1"
                && identity.run_id == "foreground-run-1"
                && identity.expected_generation == 1
    ));
    Ok(())
}
...
fn sync_agent_task(
    ...
    let thread_status = match child_status {
        ...
        sigil_kernel::TaskChildSessionStatus::Started => sigil_kernel::AgentThreadStatus::Started,
    };
...
fn agent_rename_filters_and_persists_display_name() -> Result<()> {
    ...
    assert_eq!(app.active_agent_label(), "review workspace");
...
fn agent_sidebar_rows_show_plan_subagent_availability_and_child_sessions() -> Result<()> {
    ...
    let child_ref = sigil_kernel::SessionRef::new_relative("children/thread_child_1.jsonl")?;
    ...
    let profile_id = sigil_kernel::AgentProfileId::new("reviewer")?;
    let snapshot_id = sigil_kernel::AgentProfileSnapshotId::new("snapshot_reviewer_1")?;
    ...
    let started = SessionLogEntry::Control(ControlEntry::AgentThreadStarted(
    ...
    let captured = SessionLogEntry::Control(ControlEntry::AgentProfileCaptured(
        sigil_kernel::AgentProfileCapturedEntry {
            snapshot: sigil_kernel::AgentProfileSnapshot {
                snapshot_id,
                profile_id: sigil_kernel::AgentProfileId::new("reviewer")?,
    ...
    assert!(rows.iter().any(|row| {
        row.label == "仓库审查"
            && row.detail
                == "running · reviewer · foreground task · deepseek-v4-pro · tools scoped · workspace inherited · result pending"
            && !row.muted
    }));
    app.active_pane = PaneFocus::Activity;
    app.sidebar_selected_card = SidebarCard::Agents;
...
fn agent_sidebar_rows_project_agent_thread_entries() -> Result<()> {
    ...
    assert!(
        app.agent_sidebar_rows()
            .iter()
            .any(|row| row.label == "Kernel Mapper")
    );
...
fn alt_a_cycles_agent_view_without_activity_focus() -> Result<()> {
    ...
    assert_eq!(
        app.last_notice(),
        Some(
            "agent focus: 仓库审查 · started · subagent_read · background task · deepseek-v4-pro · tools scoped · workspace inherited · result pending"
        )
    );
