...
use sigil_kernel::{
    ...
    ProviderTimeoutPhase, SecretRedactor, Session, provider_status_error, read_provider_error_body,
...
impl Provider for AnthropicProvider {
    ...
    fn observe_failure(
        ...
        if error.chain().any(|cause| {
            matches!(
                cause.downcast_ref::<AnthropicProviderError>(),
                Some(AnthropicProviderError::RetryableStatus(_))
            )
        }) {
        ...
        if error.chain().any(|cause| {
            matches!(
                cause.downcast_ref::<AnthropicProviderError>(),
                Some(AnthropicProviderError::Authentication(_))
                    | Some(AnthropicProviderError::MissingApiKey)
            )
        }) {
            return sigil_kernel::ProviderFailureObservationV1::classified(
