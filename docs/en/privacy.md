...
Use the provider-specific environment variable listed in [Providers](providers.md#authentication-priority), or save the secret through the protected credential store in Quick Setup or `/config`. This does not mean a secret is never persisted anywhere. The default `file` policy and non-interactive `auto` policy use only owner-only `~/.sigil/credentials.json`; that dedicated file is protected plaintext, not encryption. Select strict `keyring` only when native system storage and its possible authentication UI are desired. Current configuration stores only an allowlisted environment-variable name or opaque stored-credential ID. Sigil never writes a newly entered key to `sigil.toml`, workspace data, model cache, session logs, snapshots, logs, or diagnostic output. Non-current configuration is rejected and is never opened by `/config` or migrated; replace it with a current template and rotate any key that may have appeared in an earlier plaintext file. `sigil doctor` reports only the credential source and readiness.
