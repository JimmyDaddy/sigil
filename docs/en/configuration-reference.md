...
| `[connections.<id>].base_url` | provider default | Exact endpoint owned by this connection. Credentialed remote endpoints require HTTPS; unauthenticated HTTP is loopback-only. |
| `[connections.<id>].credential` | setup choice | `{ source = "environment", name = "..." }`, `{ source = "stored", id = "..." }`, or `{ source = "none" }`. Stored IDs are generated by Sigil, not hand-authored. |
| `[connections.<id>].model_context_windows.<model-id>` | unset | Optional exact token limit for one model on this connection. TUI and Desktop setup/settings can add or clear it without requiring a model catalog. |
| `[connections.<id>].options` | provider default | Provider-owned, validated wire options; credential-like keys are rejected. |
...
Provider templates, credential sources, model discovery, and troubleshooting are
documented in the [Provider guide](providers.md).
...
The table shows the current schema defaults. Quick Setup may write
...
`manual + explicit_request_only`; incompatible configuration files are rejected.
...
| `[memory].writable` | `true` | Enables durable user-preference and current-project-fact tools, local cross-session retrieval, inspect, and physical forget. Mutations ask by default and run without a prompt in `danger-full-access`; explicit denies still apply. Set `false` to opt out. |
...
| `[compaction].native_carrier_enabled` | `false` | Explicit provider-native acceleration flag. Native materialization stays fail-closed until its exact-route resume contract ships, so setting this currently issues no additional provider request. |
