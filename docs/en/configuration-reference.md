...
| Section / field | Default | Purpose |
| --- | --- | --- |
| `config_version` | setup writes `2` | Selects the provider-connection schema. Future and mixed V1/V2 schemas fail closed. |
...
| `[connections.<id>].credential` | setup choice | `{ source = "environment", name = "..." }`, `{ source = "stored", id = "..." }`, or `{ source = "none" }`. Stored IDs are generated by Sigil, not hand-authored; legacy `keyring` references remain readable. |
...
The active session keeps its own resolved `connection-id/model-id` route. Changing the saved
default does not rewrite that session; switching a ready route while idle starts a fresh session.
Existing V1 `[agent].provider` plus `[providers.*]` files remain readable. `/config` reports the
legacy mode and performs migration only after explicit confirmation; migration preserves the
exact provider and model and removes migrated plaintext from V2.

Provider templates, credential sources, model discovery, and migration troubleshooting are
...
| `[task.<role>].connection` / `.model` / `.reasoning_effort` | inherits `[agent]` | Optional role-specific V2 route. `connection` and `model` must be set together; legacy `provider` is rejected in V2. |
