...
- V1 的 `sigil serve` 只允许 loopback，除 `GET /health` 外所有 route 都要求 bearer authentication。
...
应把 `sigil serve` 视为有权限的本机 control surface。V1 只接受 loopback address，并在 bind 前拒绝关闭或缺失 bearer authentication。请把 `SIGIL_HTTP_TOKEN` 保存在环境变量中，使用高熵 value，只通过 `Authorization` header 传递；不要把它写进 URL、提交到仓库的 config、日志或截图。Sigil 只打印 token environment-variable name，绝不打印 token value。

只有 `GET /health` 不要求认证，且不会返回 session data。OpenAPI、disclosure replay、session/run query、command、cancel、approval 和 SSE 都要求 bearer token。服务不使用 cookie 或 wildcard CORS，也不声称支持 remote access 或 multi-user isolation。每个 run 的 approval mode 仍须显式选择：`deny` 阻止 gated tools；`allow_readonly` 只自动允许 read-only work；`ask` 等待 authenticated approval route。
