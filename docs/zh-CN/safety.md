...
- 通过快速设置或 `/config` 保存粘贴的凭据时，密钥会进入已配置的受保护凭据存储，`sigil.toml` 只保留不透明引用。默认 `file` 与非交互 `auto` 使用 owner-only 的 `~/.sigil/credentials.json`；这个专属文件是受权限保护的明文，不是加密。只有显式 `keyring` 可能显示系统认证界面。
