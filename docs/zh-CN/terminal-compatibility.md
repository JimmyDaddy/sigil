...
Sigil 会在异步终端输入读取器启动前只查询一次真实 cursor；后续 inline viewport resize 读取 backend 内的 cursor cache，恢复的记录或刚完成的消息则通过 scrolling region 进入终端原生 scrollback，不再让同步 cursor-position 查询与异步输入读取器竞争。长任务期间即使 TUI 保持静置，也不应再因 `The cursor position could not be read within a normal duration` 直接退出。
...
在会话记录中分别拖选单行、多行和宽字符文本，松开鼠标后直接粘贴到其他位置；不应再需要额外按键。若 Sigil 报告复制失败，确认选区仍保持高亮，并可按 `Ctrl-C` 重试。没有选区时按 `Ctrl-L`，应复制最新助手回复。所有复制内容都不应包含右侧信息栏。Sigil 会尝试本机系统剪贴板，并在启用时同时尝试带 tmux/screen passthrough 的 OSC52；只有两个适配器都无法写入时才报告剪贴板不可用。
...
在每层中重复 `/doctor`、鼠标和复制检查。键位异常时设置 `keyboard_enhancement = "off"` 并重启；鼠标异常时设置 `mouse_capture = false` 并重启；OSC52 出现可见控制文本时设置 `osc52_clipboard = false`。Sigil 仍会尝试系统剪贴板，但通过 SSH 或容器时，它未必能写到宿主机剪贴板。
