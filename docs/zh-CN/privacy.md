...
如果通过 Quick Setup 或 `/config` 保存 API key，它会以明文保存到 `sigil.toml`。不要提交包含 secret 的真实 config。

`sigil doctor` 会报告 key 来源，但不会打印 key 值。

## Session Logs

默认 session 和 control state 是 `.sigil/sessions/` 下的 append-only JSONL。里面可能包含：
...
请把 session logs 当作敏感仓库本地 artifacts，分享前先 review。
