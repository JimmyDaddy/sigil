...
接受 Task 后，Sigil 会持续展示生成的步骤，可以并行执行相互独立的工作，并在最后停到验证节点。接受计划与授予工具权限是两个独立决定：接受计划不会静默允许之后的文件修改、命令、网络请求或外部工具调用。

只有精确模型路径已经通过资格检查的新安装版本，普通对话中的较大目标才可能自动进入这套 Task 流程。其他安装仍保持 chat-first；你始终可以明确使用 `/plan` 或 `/task`。运行 `sigil doctor` 可以检查当前路径。
