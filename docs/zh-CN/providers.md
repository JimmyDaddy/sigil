...
更新锁，并在清理前同时确认配置字节和恢复记录仍是用户刚刚检查的版本；`auto` 只核对其
owner-only 凭据文件，旧的原生系统记录不在该操作范围内，必须由用户显式管理。确认发布成功或
完整回滚后会删除记录；
结果不确定时，这个阻断会跨
...
`file` 与非交互 `auto` 都只使用 owner-only 的 `~/.sigil/credentials.json`；`auto` 不会查询旧的
原生系统记录。如果该文件没有当前凭据，请打开 `/config` 重新输入一次 key。严格 `keyring` 模式
