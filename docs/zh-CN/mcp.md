...
Sigil 可以通过本地 stdio 与用户根 Streamable HTTP MCP server 接入外部工具。接入后的 MCP tools、resources 和 prompts 会进入同一个 tool registry，由同一套审批、activity、session control 和 secret-egress 规则处理。Plugin manifest 仍只允许 stdio。
...
Remote MCP 只允许出现在用户根配置中，并且必须使用 HTTPS。Header name 属于公开配置 metadata；credential value 通常应来自环境变量：
...
每个 protocol message 都会在 DNS/socket 前获得新的 durable transport authorization 与 disclosure。Direct connection 会校验并 pin 完整 resolved address set；environment proxy route 会把 proxy destination 标记为 `proxy_remote`。Sigil 不跟随 redirect。OAuth challenge 只会变成可操作的 `authentication required` 状态，不会自动打开浏览器或重放请求。

`roots` 只暴露 canonical workspace root 的 `file:` URI。`elicitation` 只接受 bounded form，并复用现有 TUI form/audit flow；URL elicitation、sampling 与 tasks 仍不支持。请求交互 form、但没有 handler 的 headless CLI activation 会 fail-closed。`/doctor` 与 `/config` 只展示 safe origin、header/environment name、capability 与 fingerprint 状态，绝不展示 resolved credential。

### OAuth 认证

OAuth 与静态 `Authorization` header 或 `bearer_token_env_var` 互斥：
...
OAuth 要求 HTTPS，并且只有明确选择**登录**后才开始。Eager 或 headless 启动只报告 `authentication required`，不会自动打开浏览器。在 `/config` 中选择 server 并打开 **Authentication**。浮层可以登录、打开或复制授权 URL、粘贴完整 callback URL 完成 headless/manual return、取消、刷新、退出，或明确清除保留的本机凭据。自动 callback 只监听 IPv4 loopback interface 的随机端口。

Discovery、动态注册、token exchange、refresh 与 revoke 可能使用不同 HTTPS 目标。每次物理请求都会重新经过常规网络策略、disclosure、destination guard 与共享 budget 检查。Redirect 与自动 retry 均关闭。请求发出后收到 `401` 会把认证标记为 stale，但不会重放该请求。

Token 与 registration secret 只保存到系统原生 credential store。Sigil 不会把它们写入 TOML、session history、日志或 support bundle，也不会回退到明文文件。退出会先尝试远端 revoke；无法证明远端 revoke 时，本机凭据会保留。你可以重试，或显式选择**只清除本机**；后者不声称远端 token 已撤销。
...
只有用户根 `[[mcp_servers]]` 可以使用 `inherit_env` 或声明 Streamable HTTP。Plugin manifest 不能请求环境/credential grant 或 remote transport；discovery 会返回 typed remediation，要求把 remote MCP 移到 root config。
...
启动过程使用一个覆盖 `initialize`、`initialized` 和首次 `tools/list` 的 absolute budget。每次 tool、resource 或 prompt 调用使用当前 tool timeout。timeout 为零时使用有限的 30 秒项目默认值，过大的配置值会被限制到 24 小时硬上限。timeout、非法 frame 或超限 frame 会永久关闭当前 client generation，尝试终止对应的 process group/tree，并回收直接 child；清理不完整时会明确报告，不能伪装成成功。Windows 上的 stdio child 会在 spawn 后立即加入 native kill-on-close Job Object；assignment、terminate、wait 或 drain 失败都会如实报告，不能伪装成 clean shutdown。各类超限错误会以 resource-limit 类型返回适用 limit 和 observed lower bound 等结构化字段。
...
- `authentication required`：打开 **Authentication** 并明确选择**登录**。
- Callback 被拒绝：重新开始登录，只使用最新浏览器 tab，或粘贴完整 callback URL。
- Credential store 不可用：解锁或启用系统原生 Keychain、Credential Manager 或 Secret Service；没有明文 fallback。
- 目标被拒绝或预算耗尽：检查 Network disclosure 与 Web policy，再重试显式动作。
- 远端 revoke 失败：重试退出，或检查警告后明确清除保留的本机凭据。
