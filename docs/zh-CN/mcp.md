...
每个 protocol message 都会在 DNS/socket 前获得新的 durable transport authorization 与 disclosure。Direct connection 会校验并 pin 完整 resolved address set；environment proxy route 会把 proxy destination 标记为 `proxy_remote`。V1 不跟随 redirect，也不处理 OAuth challenge。
