...
Sigil 可以通过 stdio MCP server 接入外部工具。接入后的 MCP 工具会进入同一个 tool registry，由同一套审批、activity、session control 和 secret egress 规则处理。
...
- `required = true`：server 启动或 `tools/list` 失败会让配置构建失败。
...
当前 `notifications/progress` 会被安全忽略，不刷 timeline。后续如果做产品化 progress 展示，需要先定义节流和用户可读映射。
...
- `required` 是否需要设为 `false`，避免可选 server 阻塞主流程。
