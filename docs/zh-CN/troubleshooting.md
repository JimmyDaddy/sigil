...
Info rail 会显示 provider 返回的上一轮 prompt usage。如果 `ctx` 行提示窗口不可用，可以设置 `fallback_context_window_tokens`。soft 和 hard threshold 表示上下文压力。有限的 idle 自动化只会在 chat turn 成功结束、没有排队输入，且 DeepSeek V4 Flash 的本地 verified target admission 通过时运行；它绝不会中断 streaming work。
...
该命令会打开只读 V2 折叠预览。打开预览不会追加 compaction record，也不会改写 session history；本地 admission 可用时，按 `Enter` 才会追加 V2 checkpoint lifecycle，否则弹窗会说明 apply 不可用的原因。自动 admission 失败只会进入 cooldown；自动 lifecycle 失败会对不变的 fold material 和 target policy 保持 latch。
