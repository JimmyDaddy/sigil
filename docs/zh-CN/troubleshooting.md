...
该命令会打开只读 V2 折叠预览。打开预览不会追加 compaction record，也不会改写 session history。如果审查显示 target ready，按 `Enter` 可确认一次手动应用；否则审查会说明缺少哪项本地 admission 前提。idle 与 queued pre-turn 路径可以使用同一 exact local admission；overflow recovery 仍不会改变 active boundary。
