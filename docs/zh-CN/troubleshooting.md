...
Info rail 会显示 provider 返回的上一轮 prompt usage。如果 `ctx` 行提示窗口不可用，可以设置 `fallback_context_window_tokens`。soft 和 hard threshold 表示上下文压力。idle、pre-turn 和 overflow compaction apply 仍然冻结，因此自动路径不会改变 session context，也不会发起恢复请求。
...
该命令会打开只读 V2 折叠预览。打开预览不会追加 compaction record，也不会改写 session history。如果审查显示 target ready，按 `Enter` 可确认一次手动应用；否则审查会说明缺少哪项本地 admission 前提。自动与 overflow 路径仍不会改变 active boundary。
