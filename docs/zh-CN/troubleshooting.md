...
Info rail 会显示 provider 返回的上一轮 prompt usage。如果 `ctx` 行提示窗口不可用，可以设置 `fallback_context_window_tokens`。soft 和 hard threshold 表示上下文压力。一次成功 turn 到达 hard threshold 并完全进入 idle 后，Sigil 可以在后台准备一次通过本地验证的 compaction。pre-turn 与 overflow recovery 仍保持冻结。
...
该命令会打开只读 V2 折叠预览。打开预览不会追加 compaction record，也不会改写 session history。如果审查显示 target ready，按 `Enter` 可确认一次手动应用；否则审查会说明缺少哪项本地 admission 前提。idle 路径可在完成的 hard-threshold turn 后使用同一 exact local admission；pre-turn 与 overflow 路径仍不会改变 active boundary。
...
该命令会在下载 public artifact 前打印 network disclosure。安装 tokenizer 不会解除冻结，也不会应用 compaction。
