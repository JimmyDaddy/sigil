...
Info rail 会显示 provider 返回的上一轮 prompt usage。如果 `ctx` 行提示窗口不可用，可以设置 `fallback_context_window_tokens`。soft 和 hard threshold 表示上下文压力。当前 compaction apply 已冻结，因此 idle 自动化不会改变 session context，也不会发起恢复请求。
...
该命令会打开只读 V2 折叠预览。打开预览不会追加 compaction record，也不会改写 session history。当前 apply 已冻结；按 `Enter` 只会关闭预览，自动和 overflow 路径也都不会改变 active boundary。

可以提前安装为后续 portable compaction 准备的、经过 checksum 验证的 DeepSeek V4 Flash tokenizer：
