...
| `/plan continue` | 显式继续最近一个未完成的计划任务 |
...
普通 composer 输入仍保持 chat-first。遇到较大的任务时，可以用 `/plan <任务>` 让 Sigil 先拆成 durable steps，再逐步执行。
...
恢复 session 只会重建可见 task 状态，不会自动继续未完成任务。需要继续时输入 `/plan continue`。
