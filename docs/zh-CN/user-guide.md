...
按 `F1` 或 `/` 打开帮助与命令；`F2` 显示或隐藏信息栏，`Shift-F2` 切换详细程度。`Ctrl-G` 聚焦活动，`Alt-V` 聚焦任务验证，`Ctrl-R` 打开最近一次受控恢复，`Alt-S` 打开当前 Intent Stack，`Ctrl-T` 展开或折叠当前任务列表、推理过程与活动。折叠的任务列表也会显示可点击的 `+N more tasks` 展开行。未选中文本时，`Ctrl-C` 取消运行；`Esc` 关闭当前浮层。完整键位表见[参考](reference.md#tui-键位)。
...
使用 `/plan` 获取只读计划；确认需要执行且计划内容合适时，再接受可审阅的 Plan 卡片。可读 Plan 可以独立审阅、修订或保存，不会提前获得执行权限。结构化 Plan提交只用于增强卡片展示，并非硬前置：模型只能返回完整文本时，Sigil会把有界文本直接保存为 Plan，而不是让生成失败。点击“运行”会原子地批准精确 Plan、创建唯一稳定的 Task及 host生成的单一线性执行单元，并立即启动 runner；它不会等待模型再生成 Task DAG或另一份结构化 contract。执行期间真实发生的 provider、权限、工具或验证问题仍会作为可恢复的 Task状态保留。希望直接进入 durable Task执行时可使用 `/task`；可选 planner如果无法产出合法结构，Sigil会自动降级到同一线性执行路径，而不是在 admission阶段终止。普通对话不会擅自继续尚未完成的任务。
