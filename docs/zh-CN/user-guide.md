...
| 聚焦 checkpoint review | `Alt-R`；随后用 `Enter` 预览/确认文件恢复，`F` fork 会话，`I` 查看证据 |
...
最近一个已完成 turn 包含受控普通文件修改时，按 `Alt-R` 聚焦 checkpoint review。第一次按
`Enter` 只会让 worker 根据 durable log 重建精确预览；timeline 会列出每个文件、恢复方向、
冲突，以及不会被覆盖的 unknown side effect。只有预览为 ready 时，再按一次 `Enter` 才会
确认恢复。
...
checkpoint review 聚焦时按 `F`，可以从这个完整 turn fork conversation。新 fork 会成为当前
