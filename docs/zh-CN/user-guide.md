...
- hard threshold：上下文压力已到临界值。修复正确性问题期间，自动压缩应用暂时冻结。
- `/compact`：打开只读 V2 折叠审查，显示 fold、keep 与 protection。修复正确性问题期间，应用暂时不可用。
