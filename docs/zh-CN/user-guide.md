...
| 聚焦 follow-up 面板 | 有 follow-up 时，在 composer 最后一行按 `Down` |
...
Sigil 正在运行时，普通 chat 输入会变成可见 follow-up，不会丢失，也不会提前写入 provider-visible history。Follow-up 会在当前 turn 完整结束后按 FIFO 自动 dispatch。`next` 只把 item 移到队首等待下一轮；`interrupt` 会先停止当前 run，再 dispatch 选中项。busy 状态下的 agent mention 不会被静默转换成 main-thread follow-up；请等待当前 turn 结束，或使用专门的 agent message 入口。
