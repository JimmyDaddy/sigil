...
`/config` 面板按 provider、permission、memory、compaction、code intelligence、terminal、appearance、Agents、Skills、Plugins 和 MCP 组织配置。`Appearance` 区块里在 `Theme` 行按 `Enter` 会循环切换内置主题并立即预览草稿 palette，`Ctrl-S` 会把选中主题保存到 `sigil.toml`。`Plugins` 区块会发现工作区里的 `.sigil/plugins/<id>/plugin.toml` manifest。
