...
| `/plan <任务>` | 先生成 durable plan，再按步骤执行复杂任务 |
| `/plan continue` | 不带额外指引地继续最近一个计划任务 |
...
`/config` 面板按 provider、permission、memory、compaction、code intelligence、terminal、Skills、Plugins 和 MCP 组织配置。`Plugins` 区块会发现工作区里的 `.sigil/plugins/<id>/plugin.toml` manifest。
...
普通 composer 输入在当前 session 没有计划任务时仍保持 chat-first。当前 session 已存在计划任务后，composer 输入会继续最近的计划任务，并作为 continuation guidance 传给当前 executor/subagent step。遇到较大的任务时，可以用 `/plan <任务>` 让 Sigil 先拆成 durable steps，再逐步执行。
...
Task run、plan、step 状态、child-session link 和 subagent approval route 摘要都会写入 append-only control entry。右侧 Info rail 会从 durable state 显示最新 task 状态、plan 版本和当前步骤。

恢复 session 只会重建可见 task 状态，不会自动继续未完成任务。需要继续时，直接在 composer 输入下一步指引；如果不需要额外指引，也可以输入 `/plan continue`。
