...
对于可写 Task 子任务，Sigil 维护一个由宿主控制的审计式工作区前沿。授权工具产生的工作区变更，
只有在对应 mutation evidence 已经结算后才会推进该前沿，因此子任务不会因一次合法编辑撤销自己的
授权。无法归因的变更、并发前沿冲突，或工具/取消绑定发生变化时仍然 fail closed。grant 中的初始
工作区快照继续作为不可变审计证据。`danger-full-access` 只取消普通审批提示，不会关闭变更审计、
取消传播或冲突检测。
...
在 TUI 中，没有文件差异的请求会把待审核的命令或工具内容放进独立的高对比区域；顶部只保留工具类型、工具名和风险，策略、副作用和隔离细节默认收在 `M` 详情中。文件写入则以具体差异作为主审核内容。没有文件变更时，TUI 不显示空的 Diff 或 Details 面板。审批回执会复述刚刚允许的动作，而不是暴露内部 call 标识。

如果工作区是 Git worktree，TUI 会在 info rail 中常驻显示当前分支和紧凑的变更计数；info rail 被隐藏或放不下时，同一状态会移到 composer 下方。Shell、terminal 和文件变更工具返回结果后会刷新该快照。

控制路由接受决定后，Desktop 与 TUI 会立即移除决定按钮并显示“正在恢复执行”；后续执行事件到达后才切换为“正在运行”。如果服务端无法确认决定是否已经送达运行实例，界面会显示“投递状态不确定”、禁用重复决定，并通过权威 run snapshot 收敛，而不会把它重新解释为待批准。交互式工具审批不会在五分钟后自动过期：它会一直等待，直到你作出决定、取消运行、关闭控制路由，或 run/session 结束。临时中断后的重试会复用同一个精确 command id 与请求身份；请求已取消或 stale，或命令、策略、执行配置发生变化时，必须重新检查。

只有 Sigil 能够为等价请求推导出有边界的语义授权时，才会显示 **本次会话允许**。该授权是 append-only 会话状态；退出后再次打开同一个 session 仍然有效。对于可识别的工作区验证，它绑定真正执行的验证命令与参数，但忽略 `tail`、`head`、`grep` 这类只改变输出展示的管道；每次具体执行仍会重新生成并校验精确 plan/hash。授权同时绑定目标、副作用上限、工作区、规范化策略 scope、执行后端、隔离配置和环境配置。它不会授权任意 Shell 命令、不同的验证步骤或验证参数、改变后的目标、远端变更、破坏性或动态代码，也不会跨越风险类别。
...
启用这一配置区块，不代表所有外部路径都安全或可访问；每条路径仍需遵守自己的规则和受保护路径检查。命令需要临时文件时，优先使用 `$SIGIL_SCRATCH_DIR`。scratch 目录归属于当前会话、仅当前用户可访问，带容量配额并在 TTL 后回收；经过运行时绑定验证的 scratch 路径不属于任意外部目录访问，因此不要求启用 `permission.external_directory`，但尝试逃逸 scratch root 的路径仍会按外部路径处理。不要把 scratch 当作长期存储。
