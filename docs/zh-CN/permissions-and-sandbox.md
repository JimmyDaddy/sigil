...
可写 Task 运行时，Sigil 会跟踪哪些工作区变更来自已批准的操作；一次合法编辑不会让 Task 自行失效。
如果文件同时出现意外变化，或权限、取消条件发生改变，Sigil 会在继续前停止，并在活动历史中保留
相关工作区状态。`danger-full-access` 只取消普通审批提示，不会关闭变更审计、取消传播或冲突检测。
